...
{
	return compareSensor(*lhs, *rhs);   // pass in value of object it points to (*(*lhs)) to an address argument (&lhs)
}										// &(*(*lhs)) = &lhs . kind of like a double negative.
...
// insertion sort will start with the left most value and compare itself to the value to its left
// the values will swap places if the value to the left is smaller and will repeat
// until the value reaches the front or until the value to the left is bigger.
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
	for (unsigned int i = 0; i < s.size(); i++) {
		int index = i;
		while (index != 0) {                               // until pivot reaches 0 
			if (comp(s[index], s[index - 1]) == true) {    // if comp(pivot, element left of pivot) == true
				swap(s[index], s[index - 1]);              // swap if true
				index--;								   // decrement pivot index
			}
			else {
				break;                                     // if comp(pivot, element left of pivot) == false, break loop
			}
		}
	}
}
...
int main()
{
	...
	// destruction time.
	{
		vector<Sensor> auxSensors(sensors);       // copy constructor

		// TODO:  Create a vector of Sensor pointers, and set each pointer
		//        to point to the corresponding Sensor in auxSensors.
		vector<Sensor*> ptrToAuxSensors;
		for (unsigned int i = 0; i < auxSensors.size(); i++) {
			ptrToAuxSensors.push_back(&auxSensors.at(i));     // push back an address of auxSensors to a vector of ptrs to Sensors
		}

		// TODO:  Sort the vector of pointers using the STL sort algorithm
		//        with compareSensorPtr as the ordering relationship.
		sort(ptrToAuxSensors.begin(), ptrToAuxSensors.end(), compareSensorPtr); // sort takes range of array to sort (front to back) and the comparison function to use
																				// the point of sorting pointers is to save cost.
		...
		sensors = auxSensors;       // assignment operator
